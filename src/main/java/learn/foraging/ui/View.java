...
View {
    ...
    public LocalDate getForageDateForItemKg() {
        displayHeader(MainMenuOption.REPORT_KG_PER_ITEM.getMessage());
        return io.readLocalDate("Select a date [MM/dd/yyyy]: ");
    }
    public LocalDate getForageDateForCatValue() {
    ...
    public void showForagersByState(List<Forager> foragers){
        if (foragers.size() == 0) {
            io.println("No foragers found");
    ...
    public String getStateAbbr(){
        ...
        while (true){
            String s= io.readRequiredString("Choose state: ");
            if(stateAbbreviations.contains(s.toUpperCase())){
               return s;
            }
            displayHeader("Enter valid state abbreviation !");
        }
    ...
    public Forager makeForager(){
        ...
        forager.setFirstName(io.readRequiredString("What is the first name: "));
        forager.setLastName(io.readRequiredString("What is the lastName: "));
        forager.setState(io.readRequiredString("What is the state: "));
        return forager;
    }
