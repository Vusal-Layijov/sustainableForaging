...
//        for (Forage f: fi)
        for(Forage f: findByDate(forage.getDate())){
            if(Objects.equals(f.getForager().getId(), forage.getForager().getId()) && f.getItem().getId()==forage.getItem().getId() && f.getDate()==forage.getDate()){
                result.addErrorMessage("Can't forage the same item on the same day. It should be tracked as one Forage.");
            }
        }
    }
